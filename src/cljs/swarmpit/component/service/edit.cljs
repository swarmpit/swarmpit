    ...
    {:state [:loading?]
     :on-success
            (fn [{:keys [response]}]
              ...
              (state/set-value (select-keys response [:repository :version :serviceName :mode :replicas :stack :agent :command :tty :user :dir :healthcheck]) settings/form-value-cursor)
