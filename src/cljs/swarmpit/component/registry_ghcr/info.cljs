(ns swarmpit.component.registry-ghcr.info
...
(defn- delete-registry-handler
  ...
  (ajax/delete
    ...
    {:on-success (fn [_]
                   ...
                   (message/info
                     (str "GitHub registry " registry-id " has been removed.")))
     :on-error   (fn [{:keys [response]}]
                   (message/error
                     (str "GitHub registry removing failed. " (:error response))))}))

(defn form-actions
  [id]
  [{:onClick #(dispatch! (routes/path-for-frontend :registry-edit {:registryType :ghcr
                                                                   :id           id}))
         ...
         (comp/grid
           ...
           (comp/grid
             ...
             (comp/card
               ...
               (comp/card-header
                 {:title     (comp/typography {:variant "h6"} "GitHub")
                  :avatar    (comp/avatar
                               {:className "Swarmpit-card-avatar"}
                               (comp/svg icon/github-path))
